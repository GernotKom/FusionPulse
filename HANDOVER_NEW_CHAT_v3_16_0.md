# FusionPulse – Übergabe für neuen Chat · v3.16.0 (Claude/Opus-Strang)
...
>    (`v3.16.0 · Worker 3.16.0`). Erst wenn beide Nummern übereinstimmen, ist eine
...
- Aktuelles Archiv: `FusionPulse_v3.16.0.zip`
...
- Tests: `npm run check` bzw. `node tests/safety-regression.mjs` → **34 Suiten, alle grün**
...
- **Die Kopfzeile zeigt seit v3.14.5 dauerhaft beide Stände:** `v3.16.0 · Worker 3.16.0`.
...
### P-A4 — Sektor-Reserve läuft für Edelmetalle ins Leere (NEU, v3.16.0)
...
### P6 Teil 1b — Eingabemaske Termine: GEBAUT, UNGETESTET
Der Code liegt in `public/app.js` (Block `v3.16.0 · P6 Teil 1b`), Markup in `index.html`
(`#earningsEditor`), CSS in `style.css`. **Es gibt noch KEINE Testsuite dafür und keinen
Funktionsnachweis** — nach Abschnitt 13 gilt das damit als offen, nicht als fertig.
Drei Fallen sind im Code dokumentiert: die Route ersetzt die Liste (nicht ergänzen),
`earningsFor()` wirkt nur im Fenster 0–14 Tage, und die Tafel zeigt nur analysierte Titel.
...
1. `TZ=Europe/Vienna node tests/safety-regression.mjs` → alle **34** Suiten grün?
2. SHA-256 der vier Claude-Blöcke unabhängig nachrechnen, nicht dem Testlauf glauben.
3. Diff gegen dieses 3.16.0, falls der Nutzer eine spätere Version schickt.
