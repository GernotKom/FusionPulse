# FusionPulse – Übergabe für neuen Chat · v3.6.1 (Claude/Opus-Strang)

> Diese Datei ist der vollständige Kontext. ZUERST vollständig lesen, dann den Code
> auditieren, DANN erst bauen. Sie ersetzt HANDOVER_NEW_CHAT_v3.6.0.md.

## Arbeitsbasis
- Aktuelles Archiv: `FusionPulse_v3.6.1.zip` (dieses Paket)
...
- Tests: `node tests/safety-regression.mjs` (muss komplett grün sein, **jetzt 11 Suiten**)
...
- **v3.6.1 · Krypto-Konsistenz, ehrliche Heatmap, Crowd-Diagnose, sichtbares Glossar (DIESES Paket).**
...
1. `node tests/safety-regression.mjs` → alle **11** Suiten grün?
2. SHA-256 der vier Claude-Blöcke gegen die Werte oben prüfen (unabhängig, nicht nur Testlauf glauben).
3. Diff gegen dieses 3.6.1 machen, falls der Nutzer eine spätere Version schickt.
