# FusionPulse – Übergabe für neuen Chat · v3.5.9 (Claude/Opus-Strang)

> Diese Datei ist der vollständige Kontext. ZUERST vollständig lesen, dann den Code
> auditieren, DANN erst bauen. Sie ersetzt HANDOVER_NEW_CHAT_v3.5.8.md.

## Arbeitsbasis
- Aktuelles Archiv: `FusionPulse_v3.5.9.zip` (dieses Paket)
...
- Tests: `node tests/safety-regression.mjs` (muss komplett grün sein, **jetzt 9 Suiten**)
...
8. **NEU (v3.5.9):** Was nicht bewertbar ist, wird ausgewiesen und NICHT geschätzt. Eine Position ohne
...
- **v3.5.9 · Modul 2: Portfolio-Risiko & Klumpung (DIESES Paket, siehe unten).**
...
### P2b — Krypto-Seite: dasselbe Muster wie P0 (nach wie vor OFFEN, konkreter Verdacht)
...
NÄCHSTER SCHRITT: `coinHeadline(r)` analog bauen (gleiche Klemme, gleicher `blockKind`-Ansatz).
Ausserdem noch zu prüfen: Heatmap-Punktfarben und Signalton-Stufen auf denselben Bruch.
...
1. `node tests/safety-regression.mjs` → alle **9** Suiten grün?
2. SHA-256 der vier Claude-Blöcke gegen die Werte oben prüfen (unabhängig, nicht nur Testlauf glauben).
3. Diff gegen dieses 3.5.9 machen, falls der Nutzer eine spätere Version schickt.
