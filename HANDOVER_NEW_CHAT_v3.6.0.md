# FusionPulse – Übergabe für neuen Chat · v3.6.0 (Claude/Opus-Strang)

> Diese Datei ist der vollständige Kontext. ZUERST vollständig lesen, dann den Code
> auditieren, DANN erst bauen. Sie ersetzt HANDOVER_NEW_CHAT_v3.5.9.md.

## Arbeitsbasis
- Aktuelles Archiv: `FusionPulse_v3.6.0.zip` (dieses Paket)
...
- Tests: `node tests/safety-regression.mjs` (muss komplett grün sein, **jetzt 10 Suiten**)
...
- **v3.6.0 · Glossar / Laien-Erklärungen (DIESES Paket, siehe unten).**
...
1. `node tests/safety-regression.mjs` → alle **10** Suiten grün?
2. SHA-256 der vier Claude-Blöcke gegen die Werte oben prüfen (unabhängig, nicht nur Testlauf glauben).
3. Diff gegen dieses 3.6.0 machen, falls der Nutzer eine spätere Version schickt.
