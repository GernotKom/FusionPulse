# FusionPulse – Übergabe für neuen Chat · v3.16.1 (Claude/Opus-Strang)
...
>    (`v3.16.1 · Worker 3.16.1`). Erst wenn beide Nummern übereinstimmen, ist eine
...
- Aktuelles Archiv: `FusionPulse_v3.16.1.zip`
...
- Tests: `npm run check` bzw. `node tests/safety-regression.mjs` → **35 Suiten, alle grün**
...
- **Die Kopfzeile zeigt seit v3.14.5 dauerhaft beide Stände:** `v3.16.1 · Worker 3.16.1`.
...
1. `TZ=Europe/Vienna node tests/safety-regression.mjs` → alle **35** Suiten grün?
2. SHA-256 der vier Claude-Blöcke unabhängig nachrechnen, nicht dem Testlauf glauben.
3. Diff gegen dieses 3.16.1, falls der Nutzer eine spätere Version schickt.
