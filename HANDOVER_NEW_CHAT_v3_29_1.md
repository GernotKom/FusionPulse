# FusionPulse – Übergabe für neuen Chat · v3.29.1 (Claude/Opus-Strang)
...
**Aktueller Stand:** v3.29.1 · `npm run check` → **47 Suiten grün** in zwei
...
## OFFENER RUECKSTAND (nicht vergessen — hier nachtragen, nicht im Chat lassen)

**R1 · Krypto-„Score/Skope"-Fenster steht im Coins-Bereich zuletzt statt zuerst.**
Der Nutzer hat das bereits vor v3.9.0 angesprochen („Skope Fenster und Heatmap
als erstes, dann der Rest") — es ist bis heute offen. **Es fehlt ein Screenshot
des Coins-Bereichs**, um zu wissen, welche Kachel genau gemeint ist; blind
umsortieren waere geraten. ERST FRAGEN, DANN SORTIEREN.

**R2 · Farbpinsel je Kachel.** Gewuenscht: kleiner Einstellungsknopf (Pinsel)
IN jeder Kachel statt einer zentralen Liste in den Einstellungen. Braucht:
Knopf-Injektion in `paintPanel()`, ein kleines Farb-Popover, Persistenz ueber
den bestehenden `--tint-…`-Mechanismus. Umfang: eine eigene Version.

**R3 · 90-Sekunden-Frischesperre** — Entscheidung des Nutzers offen (lockert).

**R4 · Nachrichtenzeile** — ein Testaufruf gegen den Tiingo-Schluessel klaert,
ob der Endpunkt im Tarif liegt.

**R5 · Erster echter Abendlauf** der Vorabend-Liste steht weiterhin aus. Bis
dahin sind alle Schwellen in `EVE` an konstruierten Balken kalibriert.
