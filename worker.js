...
/* ============================================================== Kern-Analyse */
function analyse({ pair, c5, btc5, book, fee }) {
  ...
  if (score >= 7.4 && tradable && setupFit >= 7) light = 'green';
  else if (viable && (score >= 6.2 || premove >= 7.2)) light = 'yellow';
  ...
  if (score < 7.4) blockers.push(`Score ${r1(score)}`);
  ...
  return {
    pair, light, score: r1(score), premove: r1(premove), regime, setup, orderType,
    blockers,
    // Faktoren
    ...
    exhaustion: r1(exhaustion), liquidity: r1(liquidity),
    // Preise
...
async function runScan(key, opts = {}) {
  ...
  const pre = chosen.map((p) => {
    ...
    return analyse({ pair: p, c5: cs, btc5: p === 'BTC-EUR' ? null : btc5, book: null, fee });
  }).filter(Boolean);
  ...
  const rows = chosen.map((p) => {
    const cs = candleMap.get(p);
    if (!cs || cs.length < 60) return null;
    return analyse({ pair: p, c5: cs, btc5: p === 'BTC-EUR' ? null : btc5, book: bookMap.get(p) || null, fee });
  }).filter(Boolean);
