# FusionPulse – Übergabe für neuen Chat · v3.31.0 (Claude/Opus-Strang)

> ZUERST vollständig lesen, DANN den Code auditieren, DANN erst bauen.
> **Diese Datei heißt ab v3.31.0 fest `HANDOVER.md`** und wird fortgeschrieben,
> statt je Version eine neue anzulegen. Bis v3.31.0 lagen über hundert
> Übergabe- und Notes-Dateien im Repository; das Hochladen wurde dadurch
> unhandlich, und gelesen hat sie ohnehin niemand mehr. Die Release-Historie
> steht jetzt gesammelt in `RELEASE_NOTES.md`.
> **Neue Version = diese Datei bearbeiten, keine zweite anlegen.**
...
**5 · Kehrt eine Meldung nach „erledigt" zurück, ist die FRAGE falsch verstanden
worden** (8ac, v3.30.0). R1 war zwanzig Versionen offen: zweimal als erledigt
verbucht, fünfmal vom Nutzer erneut gemeldet. Beide „Korrekturen" waren richtig
— sie beantworteten nur eine andere Frage als die gestellte. Bei der zweiten
Wiederholung nicht die Antwort verfeinern, sondern die Frage neu aufnehmen.
...
## 0. DATEIEN IM REPOSITORY (seit v3.31.0 aufgeraeumt)

Bis v3.31.0 lagen hier ~110 Markdown-Dateien, zehn .docx, eine vollstaendige
Zweitkopie des Projekts (`FusionPulse_v3.18.0/`, 1,5 MB) und sechs veraltete
Dubletten von `app.js`/`worker.js`/`style.css` im Wurzelverzeichnis. Entfernt.

Was bleibt — und was NICHT wieder wachsen darf:

```
README.md          Installation, Deploy, Secrets, D1-Setup
HANDOVER.md        DIESE Datei. Wissensbasis. Wird fortgeschrieben.
RELEASE_NOTES.md   EINE Datei. Neue Version oben anfuegen.
package.json  wrangler.jsonc
src/  public/  tests/  scripts/  migrations/
```

**Regel:** Eine neue Version legt KEINE neue Datei an. Weder
`HANDOVER_NEW_CHAT_v3_32_0.md` noch `RELEASE_NOTES_v3_32_0.md`. Beide
bestehenden Dateien werden bearbeitet. Die alten Dubletten waren nicht nur
laestig — die Zweitkopie unter `FusionPulse_v3.18.0/` enthielt eine zweite
`worker.js` und `app.js`, und wer dort etwas gesucht hat, hat im falschen Code
gelesen.

## 1. Arbeitsbasis

- Auslieferung: ein ZIP je Version, Inhalt wie oben
...
- Jede Version: `RELEASE_NOTES.md` und `HANDOVER.md` FORTSCHREIBEN (keine neuen
  Dateien anlegen, siehe Abschnitt 0), ZIP ausliefern
...
und sie war messbar — siehe `RELEASE_NOTES.md`.
...
## 8ac · v3.30.0 — R1, und warum ein Punkt zwanzig Versionen überlebt

**Der wichtigste Abschnitt für jeden, der eine wiederkehrende Nutzermeldung
bearbeitet. Vor R2 lesen.**

### Der Befund

R1 lautete: *„Das Fenster, in dem alles über einen Coin steht, steht zuletzt
und soll oben stehen — wie bei Aktien der Fokus."*

Das wurde seit v3.9.1 als **Reihenfolge-Frage** gelesen, und als solche war es
längst erledigt: `.stage` ist seit v3.9.1 das erste Element des Kryptobereichs,
v3.26.0 hat das Band davorgesetzt. Zweimal als erledigt verbucht — der Nutzer
hat den Punkt danach fünfmal erneut gemeldet.

**Die Meldung war richtig. Geprüft wurde am falschen Ort.**

Nicht die POSITION war falsch, sondern der INHALT. Das erste Fenster enthielt
den *Plan* (Zone, Kaufsumme, CRV, Risiko, Kosten, TP1/TP2, Preisleiter). Die
*Analyse* eines Coins — Kursverlauf, die neun Faktoren, die Mikrostruktur des
Orderbuchs, die Ziel-Herkunft — lag ausschließlich im Modal `#detail`, hinter
dem LETZTEN Knopf der Karte. Im Aktienbereich steht genau dieser Inhalt seit
jeher IM Fokusfenster (Chart, Modellvergleich, Positionspanel, Kantenleiste,
Interpretation).

Diese Asymmetrie ist im Markup **unsichtbar**: dort sehen beide Bereiche
identisch aus (Band → Fokus+Heatmap → Kacheln → Liste). Deshalb hat jede
Markup-Prüfung „grün" gemeldet, und deshalb hat auch der v3.29.1-Vermerk
(„es fehlt ein Screenshot, ERST FRAGEN") nicht weitergeholfen — die Frage war
nicht *welche Kachel*, sondern *was steht drin*.

Sechster Fall der Klasse „korrekt gebaut, aber nicht dort, wo der Nutzer
hinsieht": Modul-0-Schalter (v3.9.1), Fußleiste (v3.14.4), Systemampel
(v3.14.6), Wächter-Spalte, Vorabend-Kachel (v3.29.1).

> **Merksatz, jetzt als Regel 5 ganz oben:** Kehrt eine Meldung nach einer
> „erledigt"-Meldung zurück, ist meist die FRAGE falsch verstanden, nicht die
> Antwort falsch gebaut. Bei der zweiten Wiederholung die Frage neu aufnehmen,
> nicht die Antwort verfeinern.

### Was gebaut wurde

1. **`coinScopeBlocks(r)`** in `public/app.js` — der Analyseteil aus
   `refreshDetail()` herausgelöst. **Beide** Anzeigen lesen daraus: das
   Fokusfenster (neu) und das Detailfenster (unverändert erreichbar).
   → **Wer hier etwas ändert:** ein zweiter Textbaustein wäre der Fehler aus 8f
   (`sectorLag` auf nur einem Datenpfad) und 8i (Live-Quote nur im Suchpfad),
   diesmal in der Anzeige. Ein Test verbietet, dass die Faktorzeilen ein zweites
   Mal im Quelltext stehen.
2. **`.coinscope`** als DRITTES Kind der Fokuskarte, mit `grid-column:1/-1`.
   → **Nicht entfernen.** `.focus` ist ein zweispaltiges Raster (Text |
   Preisleiter); `.ladder` hat `height:100%` bei prozentual positionierten
   Marken. Ohne die Spannung über beide Spalten zieht der lange Block die
   Preisleiter in die Länge und macht sie unlesbar. Ein Test besteht darauf.
3. **Sechs fail-closed-Lücken geschlossen**, die beim Herauslösen sichtbar
   wurden und im alten Modal seit v3.0 offen waren: `imbalance` (→ `NaN %`),
   `vwapDev`/`rsi`/`atrPct`/`costRatio` (→ `null …`), `tp2Source` (leer).
   `null * 100 = 0` hätte eine nicht gemessene Orderbuch-Schieflage als
   **ausgeglichenes Buch** ausgewiesen — Regel 2 und Regel 4 in einem.
   Der Wert, der die Abwehr hier wirklich erreicht, ist `null`, nicht `0`
   (geprüft, nicht angenommen — Lehre 8z).

### Suite 48 — und warum sie in einer eigenen Datei liegt

`tests/coin-scope.mjs`. Sie ändert **keine** bestehende Datei, weil parallel am
zweiten Strang gearbeitet wird und eine neue Suite mitten in der 327-KB-Datei
`safety-regression.mjs` ein sicherer Merge-Konflikt wäre.

→ **Nächster Bearbeiter, R9:** eine Zeile in `package.json` hängt sie ein:
`"test:coinscope": "node tests/coin-scope.mjs"`, dann in `check` anhängen.
**Solange das fehlt, läuft sie nicht mit `npm run check` und der Schutz für R1
ist unbemerkt aus.**

Sie nagelt bewusst **Position UND Inhalt** fest. Ein Test nur auf die Position
hätte den Fehler wieder nicht gesehen — genau so ist er zwanzig Versionen alt
geworden.

### Sieben Negativkontrollen, und die eine, auf die es ankam

Die Suite war beim ersten Lauf grün — verdächtig, nicht fertig (Lehre 8z). Alle
sieben Sabotageproben lassen sie fallen; Protokoll in
`RELEASE_NOTES.md`. Sicherungen nach dem Lauf gelöscht (Regel aus
v3.16.0/v3.18.0).

**Die lehrreiche ist 4b.** Probe 4 (Faktoren ganz entfernt) ließ die Suite
fallen — aber an einer QUELLTEXT-Prüfung. Ob der *ausgeführte* Nachweis
überhaupt Zähne hat, war damit nicht gezeigt. Probe 4b lässt den Quelltext
unverändert (`${false?factor(…):''}`) und schaltet nur die Ausgabe ab; erst
dieser Lauf beweist, dass der ausgeführte Teil etwas prüft.

> **Merksatz:** Wenn zwei Prüfungen dieselbe Sabotage fangen können, fällt der
> Test an der einfacheren — und die schwierigere bleibt ungeprüft. Für jeden
> Schutz eine Sabotage suchen, die NUR er sehen kann. (Dieselbe
> Verallgemeinerung wie 8q/8r, jetzt zum dritten Mal.)

### Versionswechsel — warum er nicht optional war

Der Auftrag lautete „ändere ausschließlich die drei public-Dateien". Die Version
wurde trotzdem auf **3.30.0** gehoben und `npm run sync-version` gefahren.
Grund: ohne Versionswechsel behält jedes Asset seine `?v=3.29.2`-Adresse, und
der Service Worker liefert diese seit v3.19.0 **cache-first** aus — der Nutzer
hätte die Änderung nicht zu sehen bekommen. Zusätzlich berührt:
`package.json`, `src/version.js`, `public/sw.js`, `public/version.js`,
`README.md`, `wrangler.jsonc`. **`src/worker.js` NICHT** — der zweite Strang
ist unberührt, und `sync-version.mjs` fasst diese Datei ohnehin nicht an.

### Nachweise

- `npm run check` → 47 Suiten grün, `TZ=Europe/Vienna` und `TZ=America/Chicago`
- `node tests/coin-scope.mjs` → Suite 48 grün, beide Zeitzonen
- `npm run audit:reach` → kein Scrollbereich mit Bedienelementen ohne Absicherung
- SHA-256 der vier Claude-Blöcke **unabhängig nachgerechnet**, alle vier
  identisch: `1a6acdf2… · 52f69351… · de85b209… · 9e6b5efc…`

### Bewusst NICHT gemacht

- **Modal und `Details`-Knopf bleiben** (Invariante 6). Das Modal enthält
  zusätzlich den Journal-Eintrag.
- **Kein Auf-/Zuklappen.** Ein `<details>` springt bei jedem Scan (15–30 s
  `innerHTML`-Neubau) in den Ausgangszustand zurück und verwirft den Klick —
  derselbe Mechanismus, der in v3.16.1 den Tippfokus der Terminmaske gefressen
  hätte. Erst wenn der Nutzer die Länge bemängelt, und dann mit gemerktem
  Zustand.
- **Coin-Suchleiste nicht verschoben** → R8. Sie ist eine echte Abweichung vom
  Aktienbereich, war aber nicht gefragt. Eintragen statt heimlich mitmachen.
- **Kein Glossar-Eintrag neu verdrahtet.** Es ist kein neuer Fachbegriff
  hinzugekommen — die Begriffe sind wortgleich die des Detailfensters. P-D
  bleibt offen, wie es war.
- **Kein Farbregler für den Skope-Block.** `#focus` ist keine
  `data-tile`-Kachel; ein halber Eintrag wäre der Zustand, den v3.29.0 bei
  `ride` gefunden hat (Name im CSS, Ton nie gelesen).

### NÄCHSTER SCHRITT

**Rückmeldung des Nutzers einholen, ob das Fenster jetzt das ist, was gemeint
war.** Der Punkt war zweimal als erledigt verbucht und war es nicht; ein
Bildschirmfoto des Coins-Bereichs nach dem Ausrollen ist die schnellste
Klärung. Erst danach R2 (Farbpinsel je Kachel) oder R6 (Lab-Auswertungen für
Coins) angehen — R6 ist der inhaltlich größte offene Posten.

---

## 8ad · v3.31.0 — das Bandbreiten-Audit, und was davon hierher gehört

**Anlass:** `FusionPulse_Audit_Claude.md` (ChatGPT-Strang, 30.08.2026).
Kernbefund dort: Tiingo lehnt mit HTTP 429 ab, weil die **Monatsbandbreite**
(40/40 GB) erschöpft ist — nicht wegen der Abfragezahl. Hauptverdächtiger: der
vollständige `/iex`-Whole-Market-Snapshot, überschlägig ~48 Downloads je Stunde.

### Arbeitsteilung — bitte nicht durcheinanderbringen

Das Audit sagt selbst zweimal, dass es an den **Worker**-Strang gerichtet ist
...
symbolbegrenzter `/iex`-Abruf (§10 A, §14.2), Session-Taktung (§10 B, §15),
BOATS-Messung (§10 C, §17), Bandbreiten-**Messung** (§10 D), Provider-Failover
(§22–§34) und R4.

Im Public-Bereich liegt genau §28, §29 und die **Anzeigeseite** von §10 D. Das
ist v3.31.0.

### Der Befund, den das Umsetzen zutage gefördert hat
...
**Twelve Data** ausgewiesen — auch ein leeres Feld, auch **Alpaca**. Der in
§22–§34 geplante Failover hätte in der Kopfzeile den falschen Anbieter genannt.
Derselbe Fehler ein zweites Mal in `RESOURCE_LABEL` (`stocks: 'Aktien (Twelve
Data)'`, obwohl Tiingo primär liefert).

**Zweiter Befund:** dass der IEX-Feed nur rund **2–3 % des US-Volumens** sieht,
stand seit v3.8.1 in den Notizen — es war der Grund für die Korrektur von
`MOM_MIN_DOLLARVOL` von 20 auf 2 Mio. $ — und war für den Nutzer **nie
sichtbar**. Wer einen Umsatzwert liest, ohne das zu wissen, liest ihn um den
Faktor 30–50 falsch.

### Was gebaut wurde

1. **`feedInfo(meta, opening)`** — eine Wahrheitsquelle für Anbieter, Rolle und
   Marktbreite. **Es gibt bewusst keinen `else`-Zweig, der einen Namen setzt.**
   Unbekannt wird nie `primary` und nie `full`; „nicht bestimmbar" ist von
   „vollständig" sprachlich getrennt (dieselbe Trennung wie „nicht bewertbar ≠
   neutral" aus 8x). Kein Alpaca-Tarifname im Code (§33.8) — die Breite kommt
   aus dem gemeldeten Feed (`SIP` → voll, `IEX` → eingeschränkt).
2. **`#stockFeed`** im Aktienkopf, sichtbar. Die Herkunft stand bisher nur in
   einer Fußnote UNTER der Liste.
3. **`bandwidthNote(meta)`** — fail-closed. Die Zahlen liefert der Worker heute
   noch nicht. Fehlt ein Feld, steht dort **„nicht gemessen"** samt dem Satz,
   dass daraus keine Reserve folgt. Nie eine 0. Sobald der Worker liefert,
   rechnet die Anzeige ohne weitere Änderung; 40/40 GB ist **rot**.
4. **Rate-Limit-Text** bei Aktien nennt die Monatsbandbreite als mögliche
   Ursache (Lehre 8aa — Warten hilft dagegen nicht).

### Die Negativkontrolle, die den Test blind gefunden hat

Sieben Sabotageproben, Protokoll in `RELEASE_NOTES.md`. **NK3 lief beim
ersten Anlauf durch.** Ich hatte nur `feedInfo({}, {})` geprüft — „gar nichts
bekannt". Das trifft den Alpaca-Zweig nicht, also konnte eine Sabotage genau
dort nicht auffallen. Die Lücke ist real: sie beschreibt den Zustand *Anbieter
bekannt, Feed noch nicht geladen*, der beim Start jedes Mal auftritt.

> **Dritte Wiederholung der Lehre aus 8z:** ein Test gegen eine Fehlerklasse
> muss den Wert benutzen, der die Abwehr TATSÄCHLICH erreicht. Sonst prüft er
> einen Zweig, den die Sabotage gar nicht berührt, und bescheinigt Sicherheit
> für eine offene Lücke.

Zusätzlich: die erste Negativprüfung schlug auf meinem **eigenen
Erklärkommentar** an, der die alte Zeile zitiert — wörtlich der Fehler aus
v3.12.0. Alle Verbotsprüfungen laufen jetzt auf kommentarbereinigtem Quelltext.

### R11 — der Fallstrick beim Feed-Wechsel, den das Audit nicht nennt

**Wichtig für den Worker-Strang, bevor irgendwo `ALPACA_FEED=sip` gesetzt wird
oder ein SIP-Feed primär wird:**

`MOM_MIN_DOLLARVOL = 2 Mio. $` ist auf den **IEX-Anteil** kalibriert. Die
Herleitung steht in Abschnitt 7: der erste Entwurf mit 20 Mio. $ hätte
praktisch alles ausgesperrt, weil IEX nur 2–3 % des Volumens sieht. Ein
konsolidierter Feed liefert dieselbe Kennzahl um den Faktor 30–50 größer.
**Dieselbe Schwelle wäre dann trivial erfüllbar und das Einlassgitter faktisch
aus.**

Das ist exakt der Fehler „Schwelle am falschen Maßstab" aus Abschnitt 11
(v3.8.1) — nur in die andere Richtung, und es verstößt gegen Regel 4: ein
Feed-Wechsel darf nichts erleichtern. Wer den Feed wechselt, muss die Schwelle
im selben Schritt neu herleiten, **nicht raten**, und die Kennzahl vorher und
nachher gegeneinander messen. `dollarVol` wird seit v3.18.0 mitgeschrieben —
die Daten dafür sind da.

Der Worker hat übrigens bereits `alpacaFeed(env)` mit `ALPACA_FEED` (`iex`|
`sip`). Ein Alpaca-Plus-Upgrade braucht also keine neue Programmversion, aber
sehr wohl **eine Cloudflare-Variable und die Neuherleitung dieser Schwelle**.

### Bewusst NICHT gemacht

- **Client-Takt nicht angefasst** → R10. `setStockPoll()` prüft den
  Anbieternamen und wird bei Alpaca-Fallback träge. Jede Beschleunigung erhöht
  Abrufe; ohne Bandbreitenzahlen wäre die Änderung geraten. Das Audit sagt
  selbst: nicht raten, messen.
- **Keine Einsparungsprognose** (§19 verbietet sie ausdrücklich).
- **Kein Provider-Umschalter in der Oberfläche** (§26).

### Nachweise

47 Suiten grün in beiden Zeitzonen · Suite 48 und Suite 50 grün in beiden
